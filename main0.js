...
async function fetchAllData(params) {
    ...
    const operations = await request.allowDiskUse(true).exec();
    const metadataList = await AssetMetadata.find({asset: {$in: (operations || []).find(x => x['asset'])}}).allowDiskUse(true).sort('date _id').exec();
    const assets = await Asset.find({_id: {$in: operations.map(x => x['asset'])}}).exec();
    return {operations, assets, metadataList};
}

function operation_async_getActualMetadata(asset, metadataList) {
    const filteredMetadata = (metadataList || []).filter(x => x['asset']?.toString() === asset._id.toString());
    const groups = (filteredMetadata || []).find(m => `${m['category']}|${m['macro_type']}|${(m['macro_subtype'] || '')}`).filter((x, i, a) => a.indexOf(x) === i);
    const assetMetadataOperationList = [];
    for (const groupItem of groups) {
        const [category, macro_type, macro_subtype] = groupItem.split("|");
        const groupMetadataList = (filteredMetadata || []).filter(m => {
            return m['category'] === category && m['macro_type'] === macro_type && m['macro_subtype'] === (macro_subtype === '' ? null : macro_subtype)
        });
        if (groupMetadataList.length > 0) {
            if (groupMetadataList.length > 1) {
                groupMetadataList.sort((a, b) => {
                    return (a['date']?.getTime() || 0) > (b['date']?.getTime() || 0) ? -1 : 1
                });
            }
            assetMetadataOperationList.push(groupMetadataList[0])
        }
    }
    return assetMetadataOperationList;
}

function operation_async_getAsset(operation, assets) {
    return (assets || []).find(a => a['_id'].toString() === operation['asset'].toString());
}

function asset_async_getAssetsByCoordinates(coordinates, assets) {
    return (assets || []).find(asset => (coordinates || []).includes(`${asset['x']},${asset['y']}`)).filter(x => x._id.toString());
}

function asset_async_getNumberOfPreviousOperations(asset, operation, assetMetadataOperationList, assets, operations) {
    ...
    if (asset.type === "estate") {
        ...
        if (lands) {
            lands = lands.split("|");
            const landsAssetsIds = asset_async_getAssetsByCoordinates(lands, assets);
            nbPrevLandsFreeOps = (operations || []).filter(op => {
                return landsAssetsIds.includes(op['asset'].toString()) && op['operation_type'] === "free" && op['mvt_date'].getTime() < operation['mvt_date'].getTime()
            }).length;
            nbPrevLandsSaleOps = (operations || []).filter(op => {
                return landsAssetsIds.includes(op['asset'].toString()) && op['operation_type'] === "sale" && op['mvt_date'].getTime() < operation['mvt_date'].getTime()
            }).length;
        }
    }
...
function prettifyTxHash(txHash) {
    return txHash.substring(0, 12) + "..." + txHash.substring(txHash.length-6);
}

function linifyOperation(operation, operations, assets, metadataList) {
    const asset = operation_async_getAsset(operation, assets);
    const assetMetadataOperationList = operation_async_getActualMetadata(operation, metadataList);
    const {nbPrevFreeOps, nbPrevSaleOps, nbPrevLandsFreeOps, nbPrevLandsSaleOps} =
        asset_async_getNumberOfPreviousOperations(asset, operation, assetMetadataOperationList, assets, operations);
    const {
        amount1Raw, amount1CCy, amount1Usd, amount2Raw, amount2CCy, amount2Usd, amountTotal,
        feesRaw, feesCcy, feesUsd
...
async function main() {
    ...
    const {operations, assets, metadataList} = await fetchAllData(params);
    ...
    for (const operation of operations) {
        ...
        const line = await linifyOperation(operation, operations, assets, metadataList);
